...
///作成者：柏
///作成内容：データロード専用クラス
///最後修正内容：DataLoaderに変名、Defフォルダに移す
///最後修正者：柏
///最後修正日：2017.1.11
...
namespace _2016RPGTeamWork.Def
{
    static class DataLoader
    {
        ...
        /// <summary>
        /// ステージのNPCデータをロード  2016.12.20
        /// </summary>
        /// <param name="stage">ステージの種類</param>
        /// <returns></returns>
        public static int[,] NpcMapLoad(eStage stage) {
            int[,] mapData;
            List<string> lines = null;

            string[] splitLine;
            try {
                splitLine = File.ReadAllLines("Content/CSV/Stage_" + stage + "_NpcMap.csv", Encoding.GetEncoding("Shift_JIS"));
            }
            catch (FileNotFoundException ffe) {
